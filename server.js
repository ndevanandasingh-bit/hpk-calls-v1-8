...
const server=http.createServer(async(req,res)=>{
  try{
    ...
    if(url.pathname==='/api/health' && req.method==='GET') return json(req,res,200,{ok:true,service:'HPK Calls Quick Connect',version:'1.8.0',rooms:rooms.size,time:new Date().toISOString()});
...
server.listen(PORT,HOST,()=>console.log(`HPK Calls V1.8.0 listening on http://${HOST}:${PORT}`));
