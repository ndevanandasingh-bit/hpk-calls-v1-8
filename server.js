...
function serveFile(file,req,res){
  ...
  if(['index.html','sw.js','manifest.webmanifest'].includes(path.basename(file))) res.setHeader('Cache-Control','no-store, max-age=0'); else res.setHeader('Cache-Control','public, max-age=300');
...
const server=http.createServer(async(req,res)=>{
  try{
    ...
    if(url.pathname==='/api/health' && req.method==='GET') return json(req,res,200,{ok:true,service:'HPK Calls Quick Connect',version:'1.10.7',rooms:rooms.size,time:new Date().toISOString()});
...
server.listen(PORT,HOST,()=>console.log(`HPK Calls V1.10.7 listening on http://${HOST}:${PORT}`));
